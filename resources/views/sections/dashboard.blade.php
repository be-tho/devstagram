		...
		<div class="w-full md:w-8/12 lg:w-6/12 flex flex-col items-center md:flex-row">
			<div class="w-8/12 lg:w-6/12 px-5">
				<img src="{{$user->imagen ? asset('perfiles/'. $user->imagen) : asset('img/usuario.svg') }}"
				     alt="imagen del usuario" class="rounded-3xl"/>
			</div>
					...
					@if($user->id !== auth()->user()->id)
						@if( !$user->following( auth()->user() ))
							...
							>
								...
								@method('POST')
								<input type="submit"
								       class="bg-blue-600 text-white rounded-xl px-3 py-1 hover:bg-blue-700 cursor-pointer font-bold"
								       value="Seguir"
								>
							</form>
						@else
							...
							>
								...
								@method('DELETE')
								<input type="submit"
								       class="bg-red-600 text-white rounded-xl px-3 py-1 hover:bg-red-700 cursor-pointer font-bold"
